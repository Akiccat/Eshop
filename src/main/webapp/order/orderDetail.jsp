...
function cancelOrder(baseurl,orderId,msg){
	if(confirm(msg)){
		$.post(baseurl+"/order/cancelOrder.action",{orderId:orderId},function(result){
			if(result.success){
...
<div class="container-fluid">
	...
	<c:if test="${!empty orderDetail}">
		...
		<div class="col-md-10 col-md-offset-1">
			...
			<c:if test="${orderDetail.orderStatus==1}">
				<button class="btn btn-primary" type="button" onclick="cancelOrder('${pageContext.request.contextPath}','${orderDetail.orderId}','确定要取消这个订单吗？');">取消订单</button>


				<button class="btn btn-primary" type="button" onclick="javascript:location.href='${pageContext.request.contextPath}/order/payOrders.action?orderId=${orderDetail.orderId}'">去付款</button>
			</c:if>
			<button class="btn btn-primary" id="back" type="button" onclick="javascript:location.href='${pageContext.request.contextPath}/order/getMyOrders.action'">返回订单列表</button>
			</div>			 
		</div>
	</c:if>
	</div>
