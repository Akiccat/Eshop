...
public class OrderService {
    public static int addOrder(Order order, List<OrderDetail> orderDetails) {
        ...
        try {
            ...
            if (rs.next()) {
                orderId = Integer.parseInt(rs.getString(1));//获取当前记录的第一列数据
    ...
    public static List<Order> getAllOrders(int userId, String status) {
        ...
        try {
            ...
            ResultSet rs = pstmt.executeQuery();
            List<Map<String, String>> orders = DBUtil.getListFromRS(rs);
            ...
            if (orders != null) {
                for (Map<String, String> m : orders) {
                    ...
                    List<Map<String, String>> od = DBUtil.getListFromRS(rs);
