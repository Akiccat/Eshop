...
public class GoodService {
    ...
    public static List<Goods> getCategoods(int childid,int sort) {
        PreparedStatement pstmt=null;
        ResultSet rs=null;
        Connection con=null;
        ...
        try{
            ...
            con=DBUtil.getCon();
            if(sort == 0){
                sql="select goods_id,cate_id ,goods_name,goods_price,"
                ...
                sql="select goods_id,cate_id ,goods_name,goods_price,"
                        + "goods_discount,goods_stock,goods_date,goods_sales,goods_pic"
                        + " from t_goods where cate_id=? order by goods_discount asc ";
            }else if (sort == 2){
                sql="select goods_id,cate_id ,goods_name,goods_price,"
                        + "goods_discount,goods_stock,goods_date,goods_sales,goods_pic"
                        + " from t_goods where cate_id=? order by goods_discount DESC ";
            }else if(sort == 3){
                sql="select goods_id,cate_id ,goods_name,goods_price,"
                        + "goods_discount,goods_stock,goods_date,goods_sales,goods_pic"
                        + " from t_goods where cate_id=? order by goods_sales desc ";
            }else if(sort == 4){
                sql="select goods_id,cate_id ,goods_name,goods_price,"
                        + "goods_discount,goods_stock,goods_date,goods_sales,goods_pic"
                        + " from t_goods where cate_id=? order by goods_sales asc ";
            }
            pstmt=con.prepareStatement(sql);
