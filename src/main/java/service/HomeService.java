...
public class HomeService {
    ...
    public static List<Category> getCateList() {
        ...
        try {
            ...
            while (rs.next()) {
                ...
                while (rs_child.next()) {
                    if (rs_child.getRow() < 8) {//在home.jsp中更多美味选择卡项目限制为7个
                        Map map = new HashMap();
    ...
    public static List getSearchGoodList(String searchName) {
        PreparedStatement pstmt = null;
        ResultSet rs = null;
        Connection con = null;
        ...
        try {
            con = DBUtil.getCon();
            String sql_SearchGoods = "select * from t_goods where goods_name LIKE \"%\"?\"%\"";
            pstmt = con.prepareStatement(sql_SearchGoods);
            ...
            while (rs.next()) {
                ...
                map.put("goods_date", rs.getString("goods_date"));
                map.put("goods_sales", rs.getString("goods_sales"));
                list.add(map);
            }
            return list;
    ...
    public static List getNewGoodList() {
        PreparedStatement pstmt = null;
        ResultSet rs = null;
        Connection con = null;
        ...
        try {
            con = DBUtil.getCon();
            String sql_NewGoods = "select * from t_goods order by goods_date DESC limit 0,12";
            pstmt = con.prepareStatement(sql_NewGoods);
            rs = pstmt.executeQuery();
            List<Map<String, String>> list = new ArrayList<Map<String, String>>();
            while (rs.next()) {
                Map map = new HashMap();
